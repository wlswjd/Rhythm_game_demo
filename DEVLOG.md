...
- TMP 폰트 애셋: Dynamic 방식
  근거: 한글 완성형 11,172자를 미리 굽는 Static은 글자 목록 관리와 텍스처 크기 부담.
  대사를 ScriptableObject로 수시 수정하는 구조와 맞물려, 새 글자가 나와도 재생성 불필요.
- 글자 크기: 갈무리11 기준 12px의 정수배(12/24/36/48)만 사용
  근거: 비정수배에서 픽셀 폭이 고르지 않게 렌더링됨
- TMP 폰트 애셋 아틀라스 용량 부족으로 글자 누락 발생
  원인: 기본 설정(샘플링 90, 패딩 9, 1024x1024)에서 수용량 약 80자. 한글 대사에서 초과
  조치: 샘플링 36(기준 12의 3배), 패딩 4, 2048x2048, Multi Atlas 활성 → 수용량 약 2,100자
